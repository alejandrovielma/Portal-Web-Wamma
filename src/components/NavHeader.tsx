...
export function NavHeader ({ pathItems }: HeaderProps) {
  ...
  return (
    <header className="fixed w-full z-50">
      <div className="bg-light-tertiary text-white flex items-center justify-between h-14 sm:h-20 px-2 sm:px-0">
        ...
        {fullBreadcrumbs.length > 0 && (
          <button
            ...
            className="sm:hidden flex items-center justify-center cursor-pointer shrink-0 ml-auto mr-2 size-9 rounded-full hover:bg-white/10 transition-colors"
          >
            {isMenuOpen ? <CloseMenuSVG /> : <HamburgerSVG />}
          </button>
        )}
      ...
      {isMenuOpen && fullBreadcrumbs.length > 0 && (
        <nav aria-label="breadcrumb" className="sm:hidden bg-light-tertiary text-white flex flex-col border-t border-white/20 shadow-lg">
          <Link
            to="/"
          ...
          {fullBreadcrumbs.map((item, index) => (
            <Link
              ...
              to={item.link}
              onClick={() => setIsMenuOpen(false)}
              className="px-4 py-3 border-t border-white/10 hover:bg-white/10 transition-colors truncate"
            >
              {item.nombre}
            </Link>
          ))}
